...
uchar xdata holderchange,pwmsig0=0,pwmsig1=0,pwm0=0,pwm1=0;  //holderchange 云台信号更新 pwmsig 电平输出标识	  pwm 各路区分标识

uchar idata loopi=0,ritemp,busy=0;
char idata printtemp[30];
...
//uchar xdata  d2="#7";
//uchar xdata  d3="#11";
void d(uint pwmval1,uint pwmval2,uint pwmval3)
{
    sprintf(printtemp," #6 P%d #7 P%d #11 P%d \r\n  ",pwmval1,pwmval2,pwmval3) ;
    SendString(printtemp);

}
void b(uint pwmval1,uint pwmval2,uint pwmval3)
{
    sprintf(printtemp," #0 P%d  #1 P%d  #2 P%d \r\n  ",pwmval1,pwmval2,pwmval3) ;
    SendString(printtemp);

}

void c(uint pwmval1,uint pwmval2,uint pwmval3)
{
    sprintf(printtemp," #3 P%d  #4 P%d  #5 P%d \r\n  ",pwmval1,pwmval2,pwmval3) ;
    SendString(printtemp);

}

void a(uint pwmval1,uint pwmval2,uint pwmval3)
{
    sprintf(printtemp," #9 P%d #10 P%d  #8 P%d \r\n  ",pwmval1,pwmval2,pwmval3) ;
    SendString(printtemp);
...
void move(uchar ac,double x,double y,double z)             //ac=1->a   ac=2->b 
{
    if(ac==1)
    {
       ...
       a(tempo1+ada1,tempo2+ada2,tempo3+ada3);
    ...
    if(ac==3)
    {
       ...
       tempo2=2000.0*(PI-atan(pow((pow(x,2.0)+pow(y,2.0)),0.5)/z)-acos(((pow(r1,2.0)+pow(x,2.0)+pow(y,2.0)+pow(z,2.0)-pow(r2,2.0))/(2.0*r1*pow((pow(x,2.0)+pow(y,2.0)+pow(z,2.0)),0.5)))))/PI+500.0;
       tempo3=500.0+acos((pow(r1,2.0)+pow(r2,2.0)-pow(x,2.0)-pow(y,2.0)-pow(z,2.0))/(2.0*r1*r2))*2000.0/PI;
       c(tempo1+adc1,tempo2+adc2,tempo3+adc3);
    }
    if(ac==2)
    {
       ...
       b(tempo1+adb1,tempo2+adb2,tempo3+adb3);
    ...
    if(ac==4)
    {
       ...
       tempo2=2000.0*(atan(pow((pow(x,2.0)+pow(y,2.0)),0.5)/z)+acos(((pow(r1,2.0)+pow(x,2.0)+pow(y,2.0)+pow(z,2.0)-pow(r2,2.0))/(2.0*r1*pow((pow(x,2.0)+pow(y,2.0)+pow(z,2.0)),0.5)))))/PI+500.0;
       tempo3=500.0+(PI-acos((pow(r1,2.0)+pow(r2,2.0)-pow(x,2.0)-pow(y,2.0)-pow(z,2.0))/(2.0*r1*r2)))*2000.0/PI;
       d(tempo1+add1,tempo2+add2,tempo3+add3);
...
void main()
{
    ...
    while(1)
    {
        ...
        while(s2==2)
        {
            a(1500+ada1,1500+ada2,1500+ada3);
